const { setGancheProvider, ws, http } = require('../provider')
...
function getPendingTxs() {
    ws.provider.on('pending', async txHash => {
        let tx = await ws.provider.getTransaction(txHash)
        console.log(await ws.provider.call(tx), txHash)
        // console.log(await ws.provider.send('debug_traceTransaction', ['txHash']))
    })
}
